...
uki.view.declare('uki.view.Container', uki.view.Base, function(Base) {
    ...
    this.childViews = function(val) {
        ...
        uki.each(this._childViews, function(i, child) {
        	observers[child.id()] = child._observers; //preserve the observers to re-attach if required
            this.removeChild(child);
        }, this);
        uki.each(uki.build(val), function(tmp, child) {
        	if(observers[child.id()]){
        		uki.each(observers[child.id()], function(name, val){
        			uki.each(val, function(i, fnc){
        				child.bind(name, fnc);
        			});
        		});
        		delete observers[child.id()];
        	}
